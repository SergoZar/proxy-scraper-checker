...
async def _save_geodb(
    *, progress: Progress, response: Response, task: TaskID
) -> None:
...
async def download_geodb(*, progress: Progress, session: AsyncSession) -> None:
    headers = (
    ...
    async with session.stream("GET", GEODB_URL, headers=headers) as response:
